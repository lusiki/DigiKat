...
Croatian text (č ć ž š đ, rich morphology) is the substance of the project. This rule is DEFENSIVE; the
real risk is locale-dependent reads across machines.

1. **Read every text file as UTF-8 explicitly** — `readxl` (UTF-8 by default), `read.delim(..., fileEncoding =
   "UTF-8")`, `readLines(..., encoding = "UTF-8")`. Keep explicit encodings in active scripts and lexicon loaders.
...
6. **On any genuine mojibake: STOP and report** the file + byte signature, and confirm against a clean source
   ...
   "stemmer bug" in a now-archived legacy script).
