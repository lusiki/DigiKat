---
title: "Anatomija diskursa: tematska mapa katoličkog digitalnog prostora u RH (2021-2023)"
author: "Lux"
...
word_freq_for_cloud <- nlp_results_df %>%
  ...
  filter(!lemma %in% stop_words_hr) %>% # Primjenjujemo dodatni filter
...
wordcloud(
  ...
  max.words = 150,                      # Maksimalan broj riječi za prikaz
...
Nije svako spominjanje teme jednako važno. Ovaj grafikon pokazuje koliko je tema **centralna** za članak kada se u njemu pojavi. Teme s distribucijom pomaknutom udesno (npr. `ZLOSTAVLJANJE`) su one koje, kada se pojave, dominiraju člankom i predstavljaju glavnu vijest. Teme s distribucijom bliže lijevoj strani (npr. `POLITIKA`) češće se spominju usputno, kao kontekst.
