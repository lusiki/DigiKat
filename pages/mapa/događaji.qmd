...
# Uzorak (3%, sjeme 123, 2021.–2026.) i udpipe lingvističke anotacije s usklađenim doc_id-em
...
dta_stratified_sample <- readRDS("../../data/nlp/dogadjaji_sample.rds")

# Lingvističke anotacije (udpipe) za isti uzorak — usklađene preko doc_id-a.
nlp_results_df <- readRDS("../../data/nlp/dogadjaji_tokens.rds") %>%
...
ggplot(daily_duhovnost_dynamics, aes(x = date, y = duhovnost_share)) +
  ...
  scale_color_digikat_diverging(name="Prosječni sentiment") +
...
Metodologija identificira koje su se riječi najsnažnije asocirale s lematiziranom formom Stepinac u svakoj promatranoj godini, te mjeri prosječni tonalitet tekstova u kojima se te specifične narativne kombinacije pojavljuju. Ovo omogućuje praćenje ne samo frekvencije asocijacija, već i atmosferskog tonaliteta narativnih okvira.
...
ggplot(top_associated_words_final, aes(x = total_cooccurrences, y = reorder_within(lemma, total_cooccurrences, year), fill = avg_sentiment)) +
  geom_col() +
  scale_y_reordered() +
  scale_fill_digikat_diverging(name="Prosječni sentiment\nnarativnog okvira") +
  facet_wrap(~ year, scales = "free_y") +
  theme_digikat(base_size = 14) +
  theme(strip.text = element_text(face = "bold", size = 14)) +
  labs(
    title = "Longitudinalna evolucija narativa o blaženom Alojziju Stepincu",
    subtitle = "Boja stupca otkriva atmosferski tonalitet specifičnog narativnog okvira",
