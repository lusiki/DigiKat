...
<p>Mjesečni prikaz otkriva razdoblja pojačane pozornosti. Pokazatelj na 100.000 objava pokazuje zastupljenost rasprave među svim pretraživim objavama odabrane platforme. Broj objava po mjesecu nalazi se u tablici ispod grafikona.</p>
...
<div class="figure-card"><h3 id="mp-chart-title">Mjesečna zastupljenost</h3><div id="mp-chart" role="img" aria-label="Mjesečna zastupljenost odabrane platforme"></div><p class="dkb-status">Izvor: DigiKat / Determ. Prazni kružići označavaju djelomično obuhvaćene mjesece. Prekid crte označava mjesec bez podataka.</p></div>
...
<p>Uz broj uključenih objava prikazan je ukupan broj pretraživih objava, njihova vremenska pokrivenost i zastupljenost demokršćanskih ideja.</p>

```{r}
#| output: asis
table_html(data.frame(Platforma=labels[p$platform],`Pretražive objave`=integer(p$eligible_records),`Uključene objave`=integer(p$matching_records),
  `Na 100.000 objava`=number(10*p$per_10000),`Od`=p$first_day,`Do`=p$last_day,
...
<div class="dkb-report-copy"><p class="dkb-kicker">Pregled medijskog prostora</p><h3>Demokršćanstvo u medijskom prostoru</h3><p>Tematska karta, razlike među platformama i tri mjesečna portreta rasprave. Izvještaj objašnjava kako se demokršćanstvo pojavljuje u političkim porukama, sporovima oko vrijednosti i društvenim pitanjima.</p><p class="dkb-report-meta">`r integer(report$records)` objava · `r integer(report$pages)` stranica · PDF, `r number(report$bytes/1e6)` MB · `r digikat_hr_date(report$published)`</p><a class="dkb-report-download" href="../../assets/izvjestaji/demokrscanstvo-u-medijskom-prostoru.pdf" download>Preuzmi pregled (PDF)</a></div>
</article>
<article class="dkb-report">
...
<h2>O analizi</h2>
<p>Ova je analiza istraživački resurs projekta DigiKat pri Hrvatskom katoličkom sveučilištu. Analizira tekstove medijskih objava i javnih rasprava prikupljene putem sustava Determ. Autor: <a rel="author" href="https://www.lukasikic.info/">Luka Šikić</a>.</p>
