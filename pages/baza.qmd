---
title: "Baza podataka katoličkog digitalnog medijskog prostora (2021.–2026.)"
subtitle: "Strukturirani korpus katoličkog digitalnog medijskog prostora"
date: last-modified
categories: ["710.000+ objava", "47 varijabli", "2021.–2026."]
---
...
Korpus sadrži **`r n_records`** medijskih objava prikupljenih tijekom `r time_period`, fokusiranih na katoličke teme i sadržaje u hrvatskim medijima. Korpus omogućuje analizu medijskog diskursa, tonaliteta i angažmana publike u domeni katoličkih (religijskih) tema.

<div class="metric-grid">
<div class="metric-card">
<div class="metric-value">`r n_records`</div>
<div class="metric-label">Medijskih objava</div>
</div>
<div class="metric-card">
<div class="metric-value">`r n_vars`</div>
<div class="metric-label">Varijabli</div>
</div>
<div class="metric-card">
<div class="metric-value">`r time_period`</div>
...
kable(basic_info, caption = "Osnovne karakteristike baze podataka") %>%
...
if ("AUTO_SENTIMENT" %in% names(dta)) {
  ...
  sentiment_dist[, Postotak := formatC(N / sum(N) * 100, format = "f", digits = 1, decimal.mark = ",")]
...
if (length(key_vars_exist) > 0) {
  ...
  missing_data$Nedostaje <- formatC(missing_data$Nedostaje, format = "f", digits = 1, decimal.mark = ",")
...
if (length(numeric_vars_exist) > 0) {
  ...
  if (length(all_numeric) > 0) {
    ...
    cat("Numeričke varijable nisu pronađene u skupu podataka.")
  }
}
```
...
if ("SOURCE_TYPE" %in% names(dta)) {
  ...
  source_dist[, Postotak := formatC(N / sum(N) * 100, format = "f", digits = 1, decimal.mark = ",")]
...
dta_2025_2026 <- dta[year %in% c(2025, 2026)]
...
technical_info <- data.frame(
  ...
  `Vrijednost/Opis` = c(
    ...
    "NA",
    "data.table za brzu manipulaciju velikih skupova podataka",
...
kable(technical_info,
      caption = "Tehnički detalji baze podataka i preporučeni alati",
...
all_vars <- data.frame(
  ...
  Nedostaje_pct = sapply(var_names, function(v) formatC(sum(is.na(dta[[v]])) / nrow(dta) * 100, format = "f", digits = 1, decimal.mark = ",")),
  stringsAsFactors = FALSE
)
...
kable(all_vars, 
      caption = "Potpuni popis varijabli u bazi podataka",
...
Baza podataka dostupna je na platformi Kaggle: [Hrvatski katolički digitalni medijski prostor 2021.–2026.](https://www.kaggle.com/datasets/lukasikic/croatian-catholic-digital-media-space/data)
...
> **[Šikić, Luka/Hrvatsko katoličko sveučilište]**. (2026). *Katolički digitalni medijski prostor u Hrvatskoj 2021.–2026.* Dataset sadrži `r n_records` medijskih objava iz hrvatskih medija. Pristupljeno `r Sys.Date()`.

## Dodatni resursi

- **GitHub repozitorij** — [github.com/lusiki/DigiKat](https://github.com/lusiki/DigiKat)
- **Dokumentacija** — [stranica baze podataka](https://lusiki.github.io/DigiKat/baza.html)
