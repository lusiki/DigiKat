...
# Separate single-word and multi-word roots
...
if(!is.null(pattern_single) & !is.null(pattern_multi)) {
  pattern_final <- paste(pattern_single, pattern_multi, sep = "|")
} else if(!is.null(pattern_single)) {
  pattern_final <- pattern_single
} else {
  pattern_final <- pattern_multi
}

# Assign the final pattern
pattern <- pattern_final

# Display the final regex pattern
print(pattern)
# Output:
# "\\b(protestantska reformacij|katolička crkv|abortus|anglikanc|apostolsk|ateist|baptist|blagoslov|brak|caritas|duhov|duhovnost|dogm)\\w*\\b"

# Apply the pattern to filter the data frame and include matched words
filtered_df <- data22_ %>%
  mutate(
    # Extract all matches based on the regex pattern
    matches = str_extract_all(TITLE, regex(pattern, ignore_case = TRUE))
  ) %>%
  ...
  # Create a new column 'matched_words' by collapsing the list of matches into a comma-separated string
  mutate(
    matched_words = sapply(matches, function(x) paste(unique(x), collapse = ", "))
  ) %>%
...
```{r}
filtered_df <- data22_ %>%
  mutate(match_count = str_count(TITLE, regex(pattern, ignore_case = TRUE))) %>%
  filter(match_count >= 2) %>%
  select(-match_count)
```
